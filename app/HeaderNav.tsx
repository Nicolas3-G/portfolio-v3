...
const NAV_ITEMS: { id: SectionId; href: string; label: string }[] = [
  { id: "top", href: "/#top", label: "Home" },
  { id: "experience", href: "/#experience", label: "Employment" },
...
export function HeaderNav() {
  ...
  useEffect(() => {
    ...
    SECTIONS.filter((id): id is Exclude<SectionId, "top"> => id !== "top").forEach((id) => {
  ...
  return (
    <>
      <div className="px-4 sm:px-8">
        <nav className="mx-auto flex max-w-5xl items-center justify-between gap-3 py-4">
          <Link
            href="/"
            className="min-w-0 shrink text-lg font-medium text-zinc-900 max-sm:truncate"
          >
            Nicolas Guimont
          </Link>
          <div className="hidden items-center gap-4 lg:flex">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="inline-flex items-center gap-2 border border-zinc-200/80 bg-zinc-900 px-3 py-1.5 text-xs font-medium uppercase tracking-[0.16em] text-zinc-50 shadow-[0_10px_25px_rgba(15,23,42,0.35)] transition-opacity duration-200 ease-out hover:opacity-50"
              >
                <span>Resume</span>
              </button>
              <a
          ...
          <div className="flex items-center gap-2 lg:hidden">
            <button
              type="button"
              onClick={() => setShowResume(true)}
              className="inline-flex items-center gap-2 border border-zinc-200/80 bg-zinc-900 px-2.5 py-1.5 text-[0.65rem] font-medium uppercase tracking-[0.16em] text-zinc-50 shadow-[0_10px_25px_rgba(15,23,42,0.35)] transition-opacity duration-200 ease-out hover:opacity-50"
            >
              <span>Resume</span>
            </button>
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-zinc-200/80 bg-white/80 text-zinc-700 shadow-sm transition-colors hover:border-accent-gold/60 hover:text-accent-gold"
              aria-label="GitHub"
            >
              <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden>
                <path d="M12 0C5.37 0 0 5.37 0 12a12 12 0 008.21 11.44c.6.11.82-.26.82-.58v-2.02c-3.34.73-4.04-1.61-4.04-1.61-.55-1.39-1.34-1.76-1.34-1.76-1.1-.76.08-.74.08-.74 1.22.09 1.86 1.26 1.86 1.26 1.08 1.85 2.84 1.32 3.54 1.01.11-.78.42-1.32.76-1.62-2.67-.3-5.47-1.34-5.47-5.95 0-1.31.47-2.38 1.24-3.22-.12-.3-.54-1.52.12-3.17 0 0 1.01-.32 3.3 1.23a11.5 11.5 0 016 0c2.29-1.55 3.3-1.23 3.3-1.23.66 1.65.24 2.87.12 3.17.77.84 1.24 1.91 1.24 3.22 0 4.62-2.81 5.65-5.49 5.95.43.37.81 1.1.81 2.22v3.29c0 .32.22.7.82.58A12 12 0 0024 12c0-6.63-5.37-12-12-12z" />
      ...
      {showResume &&
        ...
        createPortal(
          ...
          >
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-200/70 bg-[#F6F3EA]/95 px-4 py-2.5">
                <div className="flex shrink-0 items-center gap-2">
                  <span className="text-xs font-semibold uppercase tracking-[0.18em] text-zinc-600">
                    Resume
                  </span>
                </div>
                <div className="flex min-w-0 max-w-full flex-1 justify-end gap-2 overflow-x-auto [-ms-overflow-style:none] [scrollbar-width:none] sm:flex-none [&::-webkit-scrollbar]:hidden">
                  <button
                    type="button"
                    onClick={() => {
